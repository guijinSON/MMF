...
class plotly_stock():
    ...
    def draw_candlestick_chart(self,data,ticker,freq):
    ...
    def draw_line_chart(self,data,ticker,freq):
        y = random.choice(["Open","High","Low","Close"])
        fig = px.line(data, x='Datetime', y=y, title=f'{ticker} {y}')
        fig.write_image("fig1.png")

    def draw_bar_line_chart(self,data,ticker,freq):
        if freq in ["1d","5d","1wk","1mo","3mo"]:
            rb = [dict(bounds=["sat", "mon"])]
        else:
            rb = [
    ...
    def draw_lv1_chart(self,ticker, threshold=250):
